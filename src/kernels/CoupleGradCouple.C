 ...
 */
registerMooseObject("swimApp", CoupleGradCouple);
...
template <>
InputParameters
validParams<CoupleGradCouple>()
{
  InputParameters params = validParams<Kernel>();
  params.addClassDescription("Couple Grad Couple");
  ...
  params.addRequiredCoupledVar("coupled","The coupled variable in front");


  return params;
}

CoupleGradCouple::CoupleGradCouple(const InputParameters & parameters)
...
Real
CoupleGradCouple::computeQpResidual()
{
  // velocity * _grad_u[_qp] is actually doing a dot product
 // std::cout<<"qp = "<<_qp<<"_grad_couple[_qp]="<<_test[_i][_qp]*_grad_coupled[_qp](1)<<std::endl;
 //std::cout<<"_coupled[_qp]"<<_coupled[_qp]<<std::endl;
 //std::cout<<"_grad_coupled[_qp]"<<_grad_coupled[_qp]<<std::endl;
  return _test[_i][_qp] * (_coupled[_qp]*_component * _grad_coupled[_qp]);
}

Real
CoupleGradCouple::computeQpJacobian()
{
